...
contract PlantarumCommittees is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    // ----------------------
    // Roles
    // ----------------------
    ...
    // ----------------------
    function createCommittee(string memory _name,string memory _description,string memory _image) 
        external onlyDao 
    {
        committeeCount++;
    ...
    function approveJoinCommittee(uint256 id, address applicant) external onlyActiveCommittee(id) onlyCommitteeMember(id) {
    ...
    function rejectJoinCommittee(uint256 id, address applicant) external onlyActiveCommittee(id) onlyCommitteeMember(id) {
        require(committees[id].joinRequests[applicant], "No solicitado");
        committees[id].joinRequests[applicant] = false;
    ...
    // ----------------------
    function createCommitteeProposal(uint256 id,string memory title,string memory description,uint256 deadline,string memory hashId,string memory fileHash) 
        external onlyCommitteeMember(id) onlyActiveCommittee(id) 
    {
        ...
        committeeProposals[committeeProposalCount] = CommitteeProposal(
            committeeProposalCount,id,msg.sender,title,description,"vigente",
            block.timestamp,deadline,hashId,fileHash,0,0
        );
    ...
    function voteCommitteeProposal(uint256 propId,bool support) external {
        ...
        if(support){cp.votesFor++;}else{cp.votesAgainst++;}
    ...
    function executeCommitteeProposal(uint256 propId) external {
        ...
        if(cp.votesFor>cp.votesAgainst){cp.status="aceptada";}else{cp.status="rechazada";}
    ...
    function getAllCommitteeProposals(uint256 committeeId) external view returns (CommitteeProposal[] memory) {
        uint count=0;
        for(uint i=1;i<=committeeProposalCount;i++){ if(committeeProposals[i].committeeId==committeeId){count++;} }
        CommitteeProposal[] memory list=new CommitteeProposal[](count);
        uint idx=0;
        for(uint i=1;i<=committeeProposalCount;i++){ if(committeeProposals[i].committeeId==committeeId){list[idx]=committeeProposals[i]; idx++;} }
        return list;
    }

 ...
 */
export async function pinJSONToPinata(data: any): Promise<PinataResponse> {
  if (!PINATA_JWT) throw new Error("PINATA_JWT no configurado");
...
export async function pinBase64ToPinata(params: {
  filename: string;
  mimeType?: string;
  base64: string; // sin encabezado data:
  ...
  form.append(
    "file",
    new Blob([buffer], { type: params.mimeType || "application/octet-stream" }),
    params.filename
  );

...
if (!DAO_ADDRESS) throw new Error("DAO_ADDRESS no definido en .env");
 ...
 */
export async function queryDaoEvents(fromBlock?: number, toBlock?: number) {
  try {
    ...
    const decoded = logs.map((log: Log) => {
      try {
